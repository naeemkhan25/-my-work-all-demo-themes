...
function cust_assets(){
	...
	$cust_services_icon_color=get_theme_mod('cust_services_icon_color','#dd2d6a');
	$cust_icon_colo=<<<EOD
                    .service i {
                        font-size: 38px;
                        color: {$cust_services_icon_color};
                    }

EOD;
	wp_add_inline_style('cust-main',$cust_icon_colo);

}
add_action('wp_enqueue_scripts','cust_assets');
function customizer_enqueue_assets()
{
    wp_enqueue_script("customizer-js", get_theme_file_uri("/assets/js/customizer.js"), array("jquery","customize-preview"), time(), true);
}
add_action("customize_preview_init","customizer_enqueue_assets");
